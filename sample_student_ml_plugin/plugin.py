"""Frozen machine-learning EEG QC plugin for the Bash/CSV ABI.

The ABI surface is the same as the non-ML passline project:

- qc_decision_v1(window, params)
- feature_extractor_v1(window, params)

This version uses a small embedded multiclass model with fixed coefficients.
It has no third-party dependency and performs inference only; training is not
